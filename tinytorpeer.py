...
def main():
    ...
    with contextlib.closing(socks.socksocket()) as s, \
        ...
        their_ip = socks.socket.gethostbyname("testnet-seed.bitcoin.schildbach.de")
        ...
        for i in range(10):
            tx = msg_tx()
            tx.tx = CTransaction(vin=[CTxIn(prevout=COutPoint(hash=os.urandom(32)))])
            send(writer, tx)
            gevent.sleep(0.1)
